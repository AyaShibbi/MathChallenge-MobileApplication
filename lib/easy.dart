...
class _EasyState extends State<Easy> {
  ...
  void generateNewEquation() {
    ...
    // Reset selected answer for a new question
    selectedAnswer = null;
  }
    ...
    Navigator.pushReplacement(
      ...
      MaterialPageRoute(
        builder: (context) => ScoreBoard(
            score: score,
            totalQuestions: (score + (lives+3))-1,
            level: "Easy Level"
        ),
      ),
    );
...
class MathEquation {
  ...
  static MathEquation generateRandomEquation() {
    final random = Random();
    int n1 = random.nextInt(11); // Random number between 0 and 10
  ...
  List<int> generateOptions() {
    ...
    while (options.length < 4) {
      int option = random.nextInt(21) - 10; // Random number between -10 and 10
